    ...
    def find_correct_model_file(self, cluster_number):
        ...
        try:
            ...
            for self.file in self.list_of_files:
                try:
                    # checking cluster number is matching to which model to select that model
                    # note : model name is appended by cluster number like RandomForest1
                    if (self.file.index(str( self.cluster_number))!=-1):
                        self.model_name = self.file
                except:       # if cluster number is not in file, then it will trigger exception. Exception continue
                    continue
